...
import { getLeads, updateLead, type LeadWithRelations } from '../lib/leads'; // Import lead functions and types
import { createQuote, getQuotes, type QuoteWithRelations } from '../lib/quotes'; // Import quote functions and types
...
function OrderKanban() {
  ...
  const [leads, setLeads] = useState<LeadWithRelations[]>([]); // Add state for leads
  const [quotes, setQuotes] = useState<QuoteWithRelations[]>([]); // Add state for quotes
  ...
  const [addingNote, setAddingNote] = useState(false);
    ...
    const kanbanColumns = [
        { status: 'förfrågan', title: 'Förfrågningar', color: 'border-gray-200 bg-gray-50', type: 'lead' },
        { status: 'offert_utkast', title: 'Offert (utkast)', color: 'border-yellow-200 bg-yellow-50', type: 'quote' },
        { status: 'öppen_order', title: 'Öppna Ordrar', color: 'border-blue-200 bg-blue-50', type: 'order' },
        { status: 'bokad_bekräftad', title: 'Bokade Ordrar', color: 'border-green-200 bg-green-50', type: 'order' },
        { status: 'ej_slutfört', title: 'Ej Slutförda', color: 'border-yellow-200 bg-yellow-50', type: 'order' },
        { status: 'redo_fakturera', title: 'Redo att Fakturera', color: 'border-purple-200 bg-purple-50', type: 'order' },
        { status: 'avbokad_kund', title: 'Avbokade', color: 'border-red-200 bg-red-50', type: 'order' }
    ];
 ...
 const loadData = async () => {
    try {
      ...
      const [ordersResult, customersResult, teamMembersResult, teamsResult, leadsResult, quotesResult] = await Promise.all([
        getOrders(DEMO_ORG_ID, filters),
        getCustomers(DEMO_ORG_ID),
        getTeamMembers(DEMO_ORG_ID),
        getTeams(DEMO_ORG_ID),
        getLeads(DEMO_ORG_ID, { status: 'new' }), // Fetch new leads
        getQuotes(DEMO_ORG_ID, { status: 'draft' })
      ]);
  ...
  const handleDragStart = (e: React.DragEvent, item: OrderWithRelations | LeadWithRelations, type: 'order' | 'lead') => {
 ...
 const handleDrop = async (e: React.DragEvent, targetStatus: string, targetType: 'order' | 'quote' | 'lead') => {
    ...
    if (type === 'order' && targetType === 'order') {
      ...
      if (order && order.status !== targetStatus) {
        await handleStatusChange(id, targetStatus as OrderStatus);
      }
      ...
      if (lead) {
        ...
        if (result.error) {
          showError('Fel', 'Kunde inte skapa offert från lead.');
        } else {
          success('Framgång', 'Offert skapad!');
          await updateLead(lead.id, { status: 'qualified' });
          loadData(); // Reload all data to reflect the changes
        }
      }
    }
  };
...
<div className="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-7 gap-4">
        {kanbanColumns.map((column) => {
          ...
          const totalValue = items.reduce((sum, item) => {
            if (column.type === 'lead') return sum + (item.estimated_value || 0);
            if (column.type === 'quote') return sum + (item.total_amount || 0);
            if (column.type === 'order') return sum + (item.value || 0);
            return sum;
        }, 0);
          ...
          return (
            <div
              ...
              onDrop={(e) => handleDrop(e, column.status, column.type as 'order' | 'quote' | 'lead')}
            >
              <div className="p-4">
                ...
                <div className="space-y-3">
                  {items.length === 0 ? (
                    <div className="text-center py-8 text-gray-500">
                      ...
                      <p className="text-sm">Inga ärenden i denna status</p>
                    </div>
                  ) : (
                    <>
                      {/* Render Leads */}
                      {columnLeads.map((lead) => (
                        ...
                        >
                          <div className="flex items-start justify-between mb-3 truncate">
                            <h4 className="font-medium text-gray-900 truncate">{lead.title}</h4>
                          </div>
                          {lead.description && (
                            <p className="text-sm text-gray-600 mb-3 line-clamp-2 break-words">{lead.description}</p>
                          )}
                      ...
                      {columnQuotes.map((quote) => (
                        ...
                        >
                          <div className="flex items-start justify-between mb-3">
                            <h4 className="font-medium text-gray-900 truncate">{quote.title}</h4>
                          </div>
                          {quote.description && (
                            <p className="text-sm text-gray-600 mb-3 line-clamp-2 break-words">{quote.description}</p>
                          )}
                          <div className="space-y-2">
                            {quote.customer && (
                              <div className="flex items-center text-sm text-gray-600">
                      ...
                      {columnOrders.map((order) => (
                        ...
                        {order.description && (
                          <p className="text-sm text-gray-600 mb-3 line-clamp-2 break-words">{order.description}</p>
                        )}
...
{/* === END OF TAB BUTTONS === */}
    ...
    <div className="bg-gray-50 rounded-lg p-4 space-y-3">
        {selectedOrder.primary_salesperson_id ? (
            <div>
                <span className="text-sm font-medium text-gray-500">Primär säljare:</span>
                <p className="text-sm text-gray-900">{selectedOrder.assigned_to?.full_name || 'Ej angivet'}</p>
            </div>
        ) : (
            <p className="text-sm text-gray-500">Ingen säljare tilldelad.</p>
        )}
        <button
            onClick={() => setShowCommissionModal(true)}
            className="w-full mt-2 px-4 py-2 bg-blue-100 text-blue-700 text-sm font-semibold rounded-md hover:bg-blue-200"
        >
            Hantera Provision
        </button>
    </div>
