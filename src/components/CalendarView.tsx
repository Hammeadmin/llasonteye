...
function CalendarView() {
  ...
  const handleLeadOutcome = async (lead: Lead | null | undefined, outcome: 'won' | 'lost') => {
    ...
    try {
        if (outcome === 'won') {
            // 1. Create a new quote from the lead
  ...
  }, [currentDate, viewMode, calendarMode, selectedUsers, selectedTeams, currentUserProfile]);
  ...
  const initializeCalendar = async () => {
    ...
    try {
      ...
      const [usersResult, teamsResult, userTeamsResult, leadsResult] = await Promise.all([
...
const getInitialEventFormState = () => ({
    ...
    location: '',
    is_recurring: false,
    recurrence_type: 'weekly' as 'weekly' | 'daily' | 'monthly',
    recurrence_interval: 1,
    recurrence_end_date: ''
});
    ...
    <div className="flex items-center bg-gray-100 dark:bg-gray-700/50 rounded-lg p-1 space-x-1">
        <button
            ...
            className={`px-3 py-1.5 text-sm font-medium rounded-md transition-colors duration-200 ${
                calendarMode === 'main' ? 'bg-white dark:bg-gray-700 text-blue-600 dark:text-blue-300 shadow-sm' : 'text-gray-500 hover:bg-white/60 dark:text-gray-400 dark:hover:bg-gray-600/50'
            }`}
        >
            Huvudkalender
        </button>
        <button
            ...
            className={`px-3 py-1.5 text-sm font-medium rounded-md transition-colors duration-200 ${
                calendarMode === 'sales' ? 'bg-white dark:bg-gray-700 text-blue-600 dark:text-blue-300 shadow-sm' : 'text-gray-500 hover:bg-white/60 dark:text-gray-400 dark:hover:bg-gray-600/50'
            }`}
        >
            Säljkalender
        </button>
        <button
            ...
            className={`px-3 py-1.5 text-sm font-medium rounded-md transition-colors duration-200 ${
                calendarMode === 'delivery' ? 'bg-white dark:bg-gray-700 text-blue-600 dark:text-blue-300 shadow-sm' : 'text-gray-500 hover:bg-white/60 dark:text-gray-400 dark:hover:bg-gray-600/50'
            }`}
        >
            Leveranskalender
        </button>
            ...
            {(calendarMode === 'delivery' || calendarMode === 'main') && (
                <div>
                    <h3 className="font-semibold text-gray-900 mb-3 flex items-center text-sm">
                        ...
                        Öppna Ordrar ({orders.filter(o => o.status === 'öppen_order').length})
                    </h3>
                    <div className="space-y-2">
                        {orders.filter(order => order.status === 'öppen_order').map(order => (
                            <div
                                key={order.id}
                                draggable
