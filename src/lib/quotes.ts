...
export const createQuoteFromLead = async (
  ...
  try {
    ...
    const quoteData = {
      ...
      status: 'draft' as const, // Create the quote as a draft
      subtotal: lead.estimated_value || 0,
      vat_amount: 0, // Assuming 0 VAT for a draft
    };

    const result = await createQuote(quoteData as any, []); // The 'any' cast is a small workaround for the extensive type
